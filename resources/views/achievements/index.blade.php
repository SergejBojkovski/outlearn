    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="mb-6 flex justify-between items-center">
                ...
                @auth
                    <a href="{{ route('achievements.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 focus:bg-indigo-700 active:bg-indigo-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                    ...
                    <form action="{{ route('achievements.index') }}" method="GET">
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <input type="text" name="search" id="search" value="{{ request('search') }}" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md" placeholder="Enter achievement name...">
                            ...
                            <div class="p-5">
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 mr-3">
                                        @if($achievement->image_url)
                                            <img src="{{ $achievement->image_url }}" alt="{{ $achievement->name }}" class="h-16 w-16 rounded-full">
                                        @else
                                ...
                                <div class="mt-4">
                                    ...
                                    <div class="flex items-center text-sm text-gray-500 mt-1">
                                        ...
                                        <span>{{ $achievement->users_count ?? 0 }} users earned</span>
                                    </div>
                                </div>
                                
                                <div class="mt-5 flex justify-end space-x-3">
                                    <a href="{{ route('achievements.show', $achievement->id) }}" class="inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded text-indigo-700 bg-indigo-100 hover:bg-indigo-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                                        View
                                    </a>
                                    @auth
                                        <a href="{{ route('achievements.edit', $achievement->id) }}" class="inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded text-amber-700 bg-amber-100 hover:bg-amber-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amber-500">
                                            Edit
                                        </a>
                                        <form action="{{ route('achievements.destroy', $achievement->id) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" onclick="return confirm('Are you sure you want to delete this achievement?')" class="inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded text-red-700 bg-red-100 hover:bg-red-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
                                                Delete
                                            </button>
                                        </form>
                                    @endauth
