                    ...
                    <form action="{{ route('admin.users.update', $user->id) }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div id="role-specific-fields" class="space-y-6">
                                ...
                                <!-- Student Fields -->
                                <div id="student-fields" class="space-y-4 {{ ($user->role && $user->role->name === 'student') ? '' : 'hidden' }}">
                                    <div>
                                        <x-input-label for="student_number" :value="__('Student Number')" />
                                        <x-text-input id="student_number" class="block mt-1 w-full" type="text" name="student_number" :value="old('student_number', $user->studentData ? $user->studentData->student_number : '')" />
                                        <x-input-error :messages="$errors->get('student_number')" class="mt-2" />
                                    </div>
                                </div>
                                
                                <!-- Professor Fields -->
                                <div id="professor-fields" class="space-y-4 {{ ($user->role && $user->role->name === 'professor') ? '' : 'hidden' }}">
                                    <div>
                                        <x-input-label for="department" :value="__('Department')" />
                                        <x-text-input id="department" class="block mt-1 w-full" type="text" name="department" :value="old('department', $user->professorData ? $user->professorData->department : '')" />
                                        <x-input-error :messages="$errors->get('department')" class="mt-2" />
                                    </div>
                                    
                                    <div>
                                        <x-input-label for="specialization" :value="__('Specialization')" />
                                        <x-text-input id="specialization" class="block mt-1 w-full" type="text" name="specialization" :value="old('specialization', $user->professorData ? $user->professorData->specialization : '')" />
