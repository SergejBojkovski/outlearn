                        ...
                        <thead>
                            <tr>
                                <th class="px-6 py-3 border-b border-gray-200 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">
                                    <a href="{{ route('categories.index', ['sort' => 'name', 'direction' => $sort === 'name' && $direction === 'asc' ? 'desc' : 'asc', 'search' => $search]) }}">
                                        Name
                                        @if ($sort === 'name')
                                ...
                                <th class="px-6 py-3 border-b border-gray-200 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">
                                    <a href="{{ route('categories.index', ['sort' => 'courses_count', 'direction' => $sort === 'courses_count' && $direction === 'asc' ? 'desc' : 'asc', 'search' => $search]) }}">
                                        Courses
                                        @if ($sort === 'courses_count')
                                            <span class="ml-1">{{ $direction === 'asc' ? '↑' : '↓' }}</span>
                                        @endif
                                    </a>
                                </th>
                                <th class="px-6 py-3 border-b border-gray-200 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">
                                    <a href="{{ route('categories.index', ['sort' => 'created_at', 'direction' => $sort === 'created_at' && $direction === 'asc' ? 'desc' : 'asc', 'search' => $search]) }}">
                                        Created At
                                        @if ($sort === 'created_at')
                                            <span class="ml-1">{{ $direction === 'asc' ? '↑' : '↓' }}</span>
                                        @endif
                                ...
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm leading-5 font-medium text-gray-900">
                                            <a href="{{ route('categories.show', $category->id) }}" class="hover:text-blue-500">
                                                {{ $category->name }}
                                            </a>
                                        </div>
                                    </td>
